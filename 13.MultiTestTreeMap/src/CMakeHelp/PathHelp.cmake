...
#递归rootdir，获得所有代码文件，保留文件树的目录结构，将代码组织到IDE的gendir文件夹下
function(SrcCodeGroupRCS rootdir gendir)
	message("\n")
	...
	foreach(dir ${dirlist})
		...
		message("${groupname}->${gendir}${groupname}     ${len}")
		if (${len} STRGREATER 0)  
			source_group("${gendir}${groupname}" FILES ${dircode})
		endif()
	endforeach()
...
function(AddSubDirRCS rootdir gendir)
	...
	foreach(dir ${dirs})
		...
		add_subdirectory(${dir} "${gendir}/${rootdir_name}${relativepath}")
		message("${gendir}/${rootdir_name}${relativepath}")
	endforeach()
...
function(AddTarget)
    ...
    set(oneValueArgs NAME FOLDER LIB_INCLUDE_DIR)
    set(multiValueArgs CODERES LIBRES)
	...
	#NAME填充为 执行AddTarget的CMakeList的所在文件夹名   FOLDER填充为CMake所在文件夹的父文件夹名
	if(${ARGS_CUREXE_AUTO_NAME_FOLDER})
		...
		get_filename_component(nowdir_father_name ${nowdir_father} NAME)
		...
		set_target_properties(${nowdir_name} PROPERTIES FOLDER ${nowdir_father_name})
	endif()
	...
	if(${ARGS_LIB})
		...
		target_include_directories(${ARGS_NAME} PUBLIC ${ARGS_LIB_INCLUDE_DIR})
	endif()
